...
void position_estimate_EKF(float ax,float ay,float pos_x,float pos_y,float dt)
{
	...
	{
		//x axis
		EST_X[0]=EST_X[0]+EST_X[1]*dt*VEL_WEIGHT+EST_X[2]*dt*dt*0.5+ned_acc.x*dt*dt*ACC_WEIGHT*0.5;
		EST_X[1]=EST_X[1]+EST_X[2]*dt+ned_acc.x*dt*ACC_WEIGHT;
		//y axis
		EST_Y[0]=EST_Y[0]+EST_Y[1]*dt*VEL_WEIGHT+EST_Y[2]*dt*dt*0.5+ned_acc.y*dt*dt*ACC_WEIGHT*0.5;
		EST_Y[1]=EST_Y[1]+EST_Y[2]*dt+ned_acc.y*dt*ACC_WEIGHT;
	}
}
